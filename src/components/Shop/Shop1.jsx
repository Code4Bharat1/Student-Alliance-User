...
export default function Printer() {
  ...
  useEffect(() => {
    const fetchPrinters = async () => {
      try {
        ...
        const filteredData = (data || []).filter(item => item.category !== "IFPD");
  ...
  return (<div className="min-h-screen bg-bg-primary py-8 px-4 sm:px-6 lg:px-8"> <div className="max-w-[1600px] mx-auto">

    ```
    {/* Header */}
    <div className="text-center mb-12 sm:mb-16">
      <h1 className="section-heading gradient-text">
        Explore advanced 3D printers and cameras for education excellence
      </h1>
    </div>

    {/* 🔍 Search Bar */}
    <div className="flex justify-end pr-10 mb-8">
      <div className=" bg-gray-200 px-4 py-2 rounded-full shadow-md w-full max-w-md">
        <input
          type="text"
                ...
                <div className="flex justify-center items-center mb-5 flex-grow">
                  {[...Array(5)].map((_, i) => {
                    ...
                    return (
                      <Star
                        key={i}
                        className={`h-5 w-5 ${i < rating ? "text-yellow-400" : "text-text-tertiary"
                          }`}
                        fill="currentColor"
                      />
                    );
